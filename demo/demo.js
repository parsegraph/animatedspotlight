...
app.get('/', async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>animatedspotlight</title>`);
  write(`</head>`);
  write(`<body>`);
  write(`<h1>animatedspotlight <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`);
  write(`<p>This library is available as JavaScript UMD module: <a href='/parsegraph-animatedspotlight.js'>parsegraph-animatedspotlight.js</a></p>`);
...
app.listen(port, () => {
  console.log(`See animatedspotlight build information at http://localhost:${port}`);
});
